...
"""
Triggered when the configuration is loaded from the CLI and environment variables. It is
supplied with a SimpleNamespace with attributes for each configuration fixture. The attributes 
will share a name with the corresponding fixture.

```python
@on_load_config
def use_config(config: SimpleNamespace):
    if config.dev_mode:
        # setup some dev_mode only feature
        ...
```
"""
