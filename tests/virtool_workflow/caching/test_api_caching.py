...
from tests.virtool_workflow.api.mocks.mock_sample_routes import TEST_CACHE
from virtool_workflow.api.caches import RemoteCacheWriter


async def test_api_caching(tmpdir):
    tmpdir = Path(tmpdir)
    cache_dir = tmpdir / "cache"
    cache_dir.mkdir()
    writer = RemoteCacheWriter(TEST_CACHE["key"], cache_dir)
